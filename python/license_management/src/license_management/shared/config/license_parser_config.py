"""License parser configuration in shared layer.

English:
Moved from infrastructure to shared layer to eliminate configuration scattering.
Provides parser configuration access for application layers.

Chinese:
从基础设施层移动到共享层以消除配置分散。为应用层提供解析器配置访问。
"""
...
class LicenseParserConfig:
    """Complete license parser configuration."""
