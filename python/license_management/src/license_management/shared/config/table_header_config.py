"""Table header configuration in shared layer.

English:
Moved from infrastructure to shared layer to eliminate configuration scattering.
Provides table configuration access for GUI and application layers.

Chinese:
从基础设施层移动到共享层以消除配置分散。为GUI和应用层提供表格配置访问。
"""
...
class TableHeaderConfig:
    ...
    field_whitelist: tuple[str, ...] = field(default_factory=lambda: ())


@lru_cache(maxsize=1)
def load_table_header_config() -> TableHeaderConfig:
    """Load table header configuration from file."""
    ...
    sqlite_columns: dict[str, str] = {}
    for field_name, value in columns_raw.items():
        if field_name not in _RECORD_FIELDS:
            continue
        sqlite_columns[field_name] = _sanitize_identifier(str(value))

    required_raw = sqlite_raw.get("required_fields")
    sqlite_required_fields = _normalize_required_fields(required_raw, sqlite_columns)

    for required in _RECORD_FIELDS[:1]:  # Only record_id is required
    ...
    gui_headers: dict[str, str] = {}
    for column_name in gui_order:
        gui_headers[column_name] = (
            str(headers_raw.get(column_name, column_name)).strip() or column_name
        )
...
def _normalize_field_whitelist(value: object) -> tuple[str, ...]:
    ...
    for item in typed:
        ...
        if field_name in _RECORD_FIELDS and field_name not in normalized:
