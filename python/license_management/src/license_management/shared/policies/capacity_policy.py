...
from ..errors.error_codes import ErrorCode, LicenseManagementError
...
class CapacityPolicyEnforcer:
    ...
    def evaluate_capacity_request(self, requested_capacity: int,
                                resource_check: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "remaining_capacity": self.current_capacity - current_metrics.current_load - requested_capacity
        }
    ...
    def _can_expand_capacity(self, current_metrics: CapacityMetrics) -> bool:
        ...
        if (self.last_expansion_time and
            time.time() - self.last_expansion_time < self.config.cooldown_period_minutes * 60):
            return False

        # Only expand in WARNING or CRITICAL state
        return current_metrics.state in [CapacityState.WARNING, CapacityState.CRITICAL]

    def _expand_capacity(self, requested_capacity: int,
                        current_metrics: CapacityMetrics) -> int:
        ...
        expansion_steps = max(1,
            int((expansion_needed + self.config.expansion_step_size - 1) /
    ...
    def _analyze_trend(self, metrics: List[CapacityMetrics]) -> Dict[str, Any]:
        ...
        volatility = max(abs(rates[i] - rates[i-1])
